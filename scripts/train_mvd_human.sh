META_INTERNAL=False torchrun --nnodes=1 --nproc_per_node=4 train.py \
--train_dataset " \
2345 @ MVDataset(split='train', ROOT='trajectories/scannet_0.3_0.7', aug_crop=16, mask_bg='rand', resolution=(512, 384), transform=ImgNorm, num_views=6, num_render_views=2, random_order = True, random_render_order = False, dps_name = 'dps_train.h5', random_nv_nr = [[6,2]])" \
--test_dataset " \
...
--model "AsymmetricCroCo3DStereoMultiView(pos_embed='RoPE100', patch_embed_cls='ManyAR_PatchEmbed', img_size=(512, 512), head_type='linear', output_mode='pts3d', depth_mode=('exp', -inf, inf), conf_mode=('exp', 1, inf), enc_embed_dim=1024, enc_depth=24, enc_num_heads=16, dec_embed_dim=768, dec_depth=12, dec_num_heads=12, GS = True, sh_degree=0, pts_head_config = {'skip':True})" \
...
--lr 0.00015 --min_lr 0.000001 --warmup_epochs 1 --epochs 20 --batch_size 1 --accum_iter 4 --save_freq 1 --keep_freq 5 --eval_freq 5 --num_workers 16 \
--output_dir outputs/MVD_human_512_linear

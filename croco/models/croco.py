...
class CroCoNet(nn.Module):
    ...
    def __init__(self, # (pos_embed='RoPE100', img_size=(224, 224), enc_embed_dim=1024, enc_depth=24, enc_num_heads=16, dec_embed_dim=768, dec_depth=12, dec_num_heads=12)
                 ...
                 m_ref_flag=False,       # whether to use same_view block
                ):
        ...
        self._set_decoder_same_view(enc_embed_dim, dec_embed_dim, dec_num_heads, dec_depth, mlp_ratio, norm_layer, norm_im2_in_dec, m_ref_flag)
        ...
        if m_ref_flag:
            for block in self.dec_same_view_blocks:
    ...
    def _set_decoder_same_view(self, enc_embed_dim, dec_embed_dim, dec_num_heads, dec_depth, mlp_ratio, norm_layer, norm_im2_in_dec, m_ref_flag):
        if m_ref_flag:
            self.dec_same_view_blocks = nn.ModuleList([
                DecoderBlock(dec_embed_dim, dec_num_heads, mlp_ratio=mlp_ratio, qkv_bias=True, norm_layer=norm_layer, norm_mem=norm_im2_in_dec, rope=self.rope, zero_init_last=True)
                for i in range(dec_depth)])
        else:
            self.dec_same_view_blocks = [None for i in range(dec_depth)]
